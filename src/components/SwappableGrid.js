...
// import {getRandomInt, checkRowsForMatch, checkColsForMatch, getAllMatches, markAsMatch, condenseColumns, flattenArrayToPairs} from '../lib/GridApi';
...
export default class SwappableGrid extends Component<{}> {
  ...
  processMatches(matches) {
    ...
    this.setState(
      state => {
        ...
        //
        return {tileDataSource: newTileDataSource};
      },
      () => {
  ...
  render() {
    ...
    return (
      <GestureRecognizer onLayout={this.onLayout.bind(this)} config={config} style={styles.gestureContainer} onSwipe={(direction, state) => this.onSwipe(direction, state)}>
